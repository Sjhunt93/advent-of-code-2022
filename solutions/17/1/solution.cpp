...
// #endif
        ...
        if (field.size() > windowSize) {
            ...
            for (size_t i = 0; i < windowSize/8; i++)
            {
                ...
                {
                    uint64_t v = (uint64_t) field[field.size()-1-row];
                    hash |= (v << r*7);
                    row++;
                }
            ...
            if (auto it = std::find(hashStore.begin(), hashStore.end(), hashkey); it != hashStore.end()) {
                std::cout << "Double hash detected: " << *it << " # " << hashkey << "\n";
                std::cout << "after round: " << rounds-windowSize << "\n";
                std::cout << "Hieght at repeating: " << heights[rounds-windowSize] << "\n";
                std::cout << std::distance(hashStore.begin(), it) << "\n";
                rounds = 100000;
