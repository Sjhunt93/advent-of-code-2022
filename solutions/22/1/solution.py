...
with open("input.txt") as f:
    ...
    for steps, rot in zip(items, rotations):
        while steps:
            print(steps, rot)
            new_pos = [(position[0] + increment[0]) % y_size, (position[1] + increment[1]) % x_size]
            print(new_pos)
            steps -= 1
            ...
            # just keep going until we wrap around (either edge)
            while coordinates[new_pos[0]][new_pos[1]] == " ":
                new_pos = [(new_pos[0] + increment[0]) % y_size, (new_pos[1] + increment[1]) % x_size]
